...
fn hash_with_portable(data: &[u8], algorithm: HashAlgorithm) -> Vec<u8> {
    match algorithm {
        HashAlgorithm::Sha1 => crypto_core::hash_sha1(data).to_vec(),
        HashAlgorithm::Sha256 => crypto_core::hash_sha256(data).to_vec(),
    }
}
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        match algorithm {
            HashAlgorithm::Sha1 => {
                ...
                unsafe { crate::crypto_simd::hash_sha1_sse41(data).to_vec() }
            }
            HashAlgorithm::Sha256 => {
                // Safety: this branch is selected only after runtime CPU feature check.
                unsafe { crate::crypto_simd::hash_sha256_sse41(data).to_vec() }
            }
        }
    }
...
fn hash_with_avx2(data: &[u8], algorithm: HashAlgorithm) -> Vec<u8> {
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        match algorithm {
            HashAlgorithm::Sha1 => {
                // Safety: this branch is selected only after runtime CPU feature check.
                unsafe { crate::crypto_simd::hash_sha1_avx2(data).to_vec() }
            }
            HashAlgorithm::Sha256 => {
                // Safety: this branch is selected only after runtime CPU feature check.
                unsafe { crate::crypto_simd::hash_sha256_avx2(data).to_vec() }
            }
        }
    }
...
mod tests {
    ...
    fn dispatch_matches_portable_sha1() {
        ...
        let expected = crypto_core::hash_sha1(payload).to_vec();
    ...
    fn dispatch_matches_portable_sha256() {
        ...
        let expected = crypto_core::hash_sha256(payload).to_vec();
